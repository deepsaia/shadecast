...
HOURLY = [
    ...
    "wind_direction_10m", "surface_pressure", "shortwave_radiation",
    "direct_radiation", "diffuse_radiation",
]

# Column order that solweig_gpu's preprocessor writes and reads back.
UMEP_COLS = [
    "iy", "id", "it", "imin", "Q*", "QH", "QE", "Qs", "Qf",
    "Wind", "RH", "Td", "press", "rain", "Kdn", "snow", "ldown",
    "fcld", "wuh", "xsmd", "lai_hr", "Kdiff", "Kdir", "Wd", "uhii",
]
MISSING = -999.0
...
def fetch_hourly(aoi: AOI, start: str, end: str) -> pd.DataFrame:
    r = requests.get(ARCHIVE, params={
        "latitude": aoi.lat, "longitude": aoi.lon,
...
def hottest_day(df: pd.DataFrame) -> str:
    ...
    for ts, t in zip(df["time"], df["temperature_2m"]):
        if t is not None and not np.isnan(t):
            d = ts.strftime("%Y-%m-%d")
            peaks[d] = max(peaks[d], t)
...
def to_umep(df: pd.DataFrame, date: str) -> pd.DataFrame:
    ...
    day = df[df["time"].dt.strftime("%Y-%m-%d") == date].copy()
    if day.empty:
        raise ValueError(f"No met rows for {date}")

    out = pd.DataFrame({c: [MISSING] * len(day) for c in UMEP_COLS})
    t = day["time"].dt
    out["iy"] = t.year.values
    out["id"] = t.dayofyear.values
    out["it"] = t.hour.values
    out["imin"] = 0
    out["Td"] = day["temperature_2m"].values                      # air temp, degC
    out["RH"] = day["relative_humidity_2m"].values                # percent
    out["Wind"] = day["wind_speed_10m"].values / 3.6              # km/h -> m/s
    out["Wd"] = day["wind_direction_10m"].values                  # degrees
    out["press"] = day["surface_pressure"].values / 10.0          # hPa -> kPa
    out["Kdn"] = day["shortwave_radiation"].values                # W/m2 global
    out["Kdir"] = day["direct_radiation"].values
    out["Kdiff"] = day["diffuse_radiation"].values
    out["rain"] = 0.0
    out["snow"] = 0.0

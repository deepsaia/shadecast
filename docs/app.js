...
/* Sections are written by render(), which runs after a fetch, so a hash already in the
   URL has nothing to resolve against when the browser first tries. Re-resolve it once
   the page exists, and accept the printed section number as well as the name. */
function gotoStep(hash){
  const want = decodeURIComponent(String(hash).replace(/^#/, ""));
  if(!want) return;
  const byNumber = /^\d{1,2}$/.test(want)
    ? document.querySelector(`.step[data-n="${want.padStart(2,"0")}"]`)
    : null;
  const target = byNumber || document.getElementById(want);
  // scrollIntoView honours the scroll-margin-top that keeps the sticky bar off the heading.
